...
function getAllWeeklyViews() {
  return weeklyRows.map(toWeeklyView);
}
...
export async function updateWeeklyData(
  ...
  await delay();
  const before = weeklyRows.find((row) => row.id === rowId);
  if (!before) throw new Error('未找到投放记录');
  const after = withUpdate({ ...before, ...patch }, operatorName);
  weeklyRows = weeklyRows.map((row) => (row.id === rowId ? after : row));
...
export async function deleteWeeklyData(rowId: string, operatorName: string) {
  await delay();
  const before = weeklyRows.find((row) => row.id === rowId);
  weeklyRows = weeklyRows.filter((row) => row.id !== rowId);
  writeAudit(
    operatorName,
    '删除',
    '本周投放',
...
export async function uploadWeeklyCsv(text: string, operatorName: string) {
  ...
  const rows = parseCsv(text).map((cols, index): WeeklyDelivery => ({
    ...
    spendAmount: Number(cols[4]) || 0,
    readCount: Number(cols[5]) || 0,
    wechatAdds: Number(cols[6]) || 0,
    dealCount: Number(cols[7]) || 0,
    dealAmount: Number(cols[8]) || 0,
    createdBy: operatorName,
    createdAt: timestamp(),
    updatedBy: operatorName,
    updatedAt: timestamp(),
...
export async function updatePlan(
  ...
  await delay();
  const before = plans.find((row) => row.id === planId);
  if (!before) throw new Error('未找到排期');
  const after = withUpdate({ ...before, ...patch }, operatorName);
  plans = plans.map((row) => (row.id === planId ? after : row));
...
export async function deletePlan(planId: string, operatorName: string) {
  await delay();
  const before = plans.find((row) => row.id === planId);
  plans = plans.filter((row) => row.id !== planId);
...
export async function uploadPlanCsv(text: string, operatorName: string) {
  ...
  const rows = parseCsv(text).map((cols, index): NextWeekPlan => ({
    ...
    plannedAmount: Number(cols[3]) || 0,
    paymentStatus: (cols[4] as PaymentStatus) || 'unpaid',
    sortOrder: plans.length + index + 1,
    createdBy: operatorName,
    createdAt: timestamp(),
    updatedBy: operatorName,
    updatedAt: timestamp(),
